...
router.get('/trips', async (req: Request, res: Response) => {
    try {
        ...
        if (status && status !== 'all') {
            filters.push(eq(tripBoards.status, status as string));
        }
        if (difficulty) {
            filters.push(eq(tripBoards.difficulty, difficulty as string));
        }
        if (destination) {
...
router.get('/trips/:id', validate(v.tripIdParamSchema, 'params'), async (req: Request, res: Response) => {
    try {
        ...
        const dateOptions = await db
            .select()
            .from(tripDateVotes)
            .where(eq(tripDateVotes.tripId, id))
        ...
        const gearItems = await db
            ...
            .where(eq(tripGearItems.tripId, id));
...
router.put('/trips/:id', validate(v.tripIdParamSchema, 'params'), validate(v.updateTripSchema, 'body'), async (req: Request, res: Response) => {
    try {
        ...
        const trip = await db.select().from(tripBoards).where(eq(tripBoards.id, id)).limit(1);
        if (trip.length === 0) {
            return res.status(404).json({ error: 'Trip not found' });
        }
...
router.post('/trips/:tripId/dates/:dateId/vote', async (req: Request, res: Response) => {
    try {
        ...
        const existingVote = await db
            ...
            .where(and(
                eq(tripDateUserVotes.dateOptionId, dateId),
        ...
        await db
            ...
            .where(eq(tripDateVotes.id, dateId));
...
router.post('/trips/:tripId/dates/:dateId/confirm', async (req: Request, res: Response) => {
    try {
        ...
        // Get selected date
        const dateOption = await db.select().from(tripDateVotes).where(eq(tripDateVotes.id, dateId)).limit(1);
...
router.put('/trips/:tripId/gear/:itemId', async (req: Request, res: Response) => {
    try {
        ...
        const updatedGear = await db
            ...
            .where(eq(tripGearItems.id, itemId))
            .returning();
...
router.delete('/trips/:tripId/gear/:itemId', async (req: Request, res: Response) => {
    try {
        ...
        const gear = await db.select().from(tripGearItems).where(eq(tripGearItems.id, itemId)).limit(1);
        if (gear.length === 0) {
            return res.status(404).json({ error: 'Gear item not found' });
        }

        await db.delete(tripGearItems).where(eq(tripGearItems.id, itemId));
...
router.get('/trips/:id/messages', async (req: Request, res: Response) => {
    try {
        ...
        const messages = await db
            ...
            .where(eq(tripMessages.tripId, id))
