...
export const EventDetailsPage: React.FC = () => {
        ...
        const fetchEvent = async () => {
            try {
                ...
                if (found) {
                    ...
                    // Store debug info
                    const availableIds = res.data.map((e: any) => `"${e.id}"`).join(', ');
                    setDebugInfo(`Requested: "${id}" | Available: [${availableIds}]`);
                }
            } catch (err) {
    ...
    if (!event) return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-6">
            ...
            {/* DEBUG INFO FOR USER */}
            <div className="mt-8 p-4 bg-gray-100 rounded-xl text-xs font-mono text-left max-w-md w-full overflow-auto text-gray-600 break-all">
                <p className="font-bold mb-1">Debug Info:</p>
                {debugInfo || 'Loading API data...'}
            </div>
        </div>
    );
